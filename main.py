...
def sign_up():
    ...
    if flask.request.method == "GET":
        ...
        username, password, password_confirmed = data.get("email"), data.get("password"), data.get("password2")
...
def booking_info(car_id):
    ...
    if flask.request.method == "POST":
        ...
        records = car_db.to_dictionary(car_db.execute_return(f"select * from {car_db.table} where {car_db.ID} = %s", (car_id, )), attribute_list=car_db.property_list)
...
def issues():
    ...
    if flask.request.method == "POST":
        ...
        if role == "engineer":
            ...
            attributes['car_id_list'] = issues_db.get_values_of_col(issues_db.CAR_ID)
            ...
            attributes['all_issues'] = records
            attributes['car_id_list'] = issues_db.get_values_of_col(issues_db.CAR_ID)
            attributes['engineer_id_list'] = issues_db.get_values_of_col(issues_db.ENGINEER_ID)
            # attributes['location_list'] = issues_db.get_values_of_col(car_db.LOCATION)
            # attributes['status'] = issues_db.get_values_of_col(issues_db.STATUS)
...
def cars_report(id):
    ...
    new_message = flask_mail.Message(f"New issues with Car {id}", 
                                        sender = flask.session.get(login_db.USERNAME),

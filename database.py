...
class UserDatabase (AbstractDatabase):
    ...
    def add_user(self, id, name, address, phone_number):
        query = f"insert into {self.table} " + " ({}, {}, {}, {}) values (%s, %s, %s, %s)".format(*self.property_list)
    ...
    def update_user(self, user_id, cols_update: tuple, new_values: tuple):
        if len(cols_update) != len(new_values):
            raise Exception(f"Number of rows and numbers of new values are not equal. " + str({len(cols_update)}) + " " + str({len(new_values)}))
        else:
            pass
...
class BookingDatabase(AbstractDatabase):
    ...
    def add_booking(self, car_id, user_id, booking_detail, from_time, to_time):
        ...
        if len(records) > 0:
            ...
            response = self.calendar.add_event(user_id, car_id, datetime.datetime.strptime(from_time, "%Y-%m-%d %H:%M:%S"), datetime.datetime.strptime(to_time, "%Y-%m-%d %H:%M:%S"), booking_detail)
...
class IssuesDatabase(AbstractDatabase):
    ...
    def add_issues(self, car_id, engineer_id, location, message):
        ...
        self.execute_no_return(query, (car_id, engineer_id, location, False, message))
...
if __name__ == "__main__":
    ...
    user_db.update_user(4, cols_update=(user_db.NAME,), new_values=("Viet Nam oi",))
    ...
    print(car_db.update_car(4, [car_db.BRAND, car_db.BODY_TYPE], ("Zoros", "Unicycle")))
    car_db.remove_car(4)
    print(car_db.get_all_car())
